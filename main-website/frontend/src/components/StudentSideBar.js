...
const SideBar = () => {
  ...
  const Menus = [
    { title: "Dashboard", icon: <DashboardIcon width='20px' height='20px' fill='white' />, route: "dashboard" },
    {
      ...
      subMenu: ["Home", "News & Events", "Upcoming Events", "Faculties", "Library", "Newsletter"],
      key: "pages",
    },
    {
      title: "Students",
  ...
  return (
    <div className={`admin-sidebar ${open ? "admin-open" : "admin-closed"}`}>
      <div className='admin-logo-section'>
        ...
        <h1 className={`college-name ${!open ? "admin-closed" : ""}`}>Student Dashboard</h1>
      </div>
              ...
              <ul className='ul-items'>
                {Menu.subMenu.map((subMenu, subIndex) => (
                  <li key={subIndex} className="li-items">
                    <Link to={`/admin-dashboard/${subMenu.toLowerCase().replace(/\s+/g, '-')}`} className="submenu-link">
                      {subMenu}
                    </Link>
                  </li>
                ))}
